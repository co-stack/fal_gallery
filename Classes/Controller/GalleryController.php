...
class GalleryController extends ActionController {
	...
	public function validateConfiguration() {
		...
		if ($this->actionMethodName === 'listAction' || $this->actionMethodName === 'categoryAction') {
			if (!isset($this->settings['default']['folder']) || count(explode(':', $this->settings['default']['folder'])) !== 3) {
	...
	protected function setImageFileExtension() {
		if (isset($this->settings['images']['extension']) && $this->settings['images']['extension'] !== '') {
			...
			if (isset($GLOBALS['TYPO3_CONF_VARS']['GFX']['imagefile_ext']) && !empty($GLOBALS['TYPO3_CONF_VARS']['GFX']['imagefile_ext'])) {
	...
	protected function setFileExtensionFilter() {
		...
		$this->selectedStorage->addFileAndFolderNameFilter(array(
	...
	protected function getErrorMessageForActionName($actionName) {
		return sprintf($this->configurationErrorMessage, $actionName, $this->errorMessageArray[$this->errorMessageArray['current']]);
	}
}
